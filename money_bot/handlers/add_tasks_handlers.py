...
from money_bot.utils import db_utils, markups, models
from money_bot.utils.states import AddTasksStates, GlobalStates
...
async def check_for_admin(query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await bot.edit_message_text(
            ADD_TASKS_MENU_TEXT["admin_failed"],
...
def register_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        check_for_admin, markups.add_tasks_factory.filter(data="0"), state=AddTasksStates.check_for_admin
    )
